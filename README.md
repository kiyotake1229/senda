...
- **スマホは画面キーボードで遊べる。** Simeji や iOS と同じ12キーのフリック入力でかなを直接打つ。右上の ABC で画面のローマ字キーボードに切り替え。端末のキーボード（外付け）も選べる（[#0016](docs/20260925_NEW_0016_APP_スマホ用の画面キーボード.md)）。キーボードの高さはつまみや設定で変えられ、片手モードもある（[#0018](docs/20260925_ENH_0018_APP_スマホ用キーボードの高さと片手モード.md)）
